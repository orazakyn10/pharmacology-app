...
TOXICITY_DATA = {
    ...
    "Olaparib": {
    "Костный мозг": {"risk": "high", "note": "Анемия, миелосупрессия — частые дозолимитирующие эффекты"},
    "ЖКТ": {"risk": "medium", "note": "Тошнота, рвота, диарея"},
    "Печень": {"risk": "low", "note": "Редкое повышение печёночных ферментов"},
    },
    "Vinorelbine": {
    "Костный мозг": {"risk": "high", "note": "Нейтропения — основная дозолимитирующая токсичность"},
    "Периферические нервы": {"risk": "medium", "note": "Периферическая нейропатия, сниженные рефлексы"},
    "ЖКТ": {"risk": "medium", "note": "Запор, тошнота, рвота, стоматит"},
    "Лёгкие": {"risk": "low", "note": "Редкие случаи одышки и интерстициальных изменений"},
},
}
...
st.caption(f"Метрики модели: F1={drug_data['metrics']['f1']:.3f}, ROC-AUC={drug_data['metrics']['roc_auc']:.3f}, порог классификации={threshold:.2f}")
